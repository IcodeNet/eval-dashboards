...
export const buildAgentQualitySetupPlaybook = (profile: AgentQualityInitProfile): ScaffoldFile => {
  ...
  return {
    ...
    content: [
      ...
      '- Default to >=2 independent reviewers per adjudication row (adjudicate export emits `reviews: [{}, {}]`).',
      '- Only use `eval-dashboards adjudicate import --allow-single-reviewer` as an explicit, documented reduced-rigor opt-out.',
      '- Record judgeModel, rubricVersion, inter-rater disagreement rate (from merge metadata), and examples of disagreements.',
      '- Do not switch a suite to blocking until calibration drift and inter-rater disagreement are both acceptable.',
      '',
...
export const renderAgentQualityTeachMode = (outputDir: string, files: ScaffoldFile[]): string => {
  ...
  return [
    ...
    '6. Judges: use LLM judges for nuanced quality, then calibrate against >=2 independent reviewers by default (single-reviewer is an explicit, reduced-rigor opt-out via --allow-single-reviewer).',
