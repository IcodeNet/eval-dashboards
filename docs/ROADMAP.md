...
- [ ] Open question, raised while reviewing the assistant-ui case study: `eval-report/v1`'s suite-level `passRate` blends rows that are *supposed* to fail (this harness's `baseline` rows — a failing baseline is the correct/expected outcome, proof the case tests something) with rows that are supposed to pass (`candidate-guidance` rows — the real signal). A flat 77.8% pass rate is misleading for this shape of harness. Not yet decided: (a) exclude a `category`/tag like `baseline-evidence` from suite pass-rate math, (b) add a distinct suite metric for "candidate pass rate excluding baseline", or (c) leave it and document that per-row category is the correct signal for A/B-shaped suites, not the suite-level rate. Needs a decision before recommending this pattern more broadly.
