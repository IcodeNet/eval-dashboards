...
- [ ] Use each skill run as an adoption-funnel event: log outcome in `docs/community-partnership-log.md`, feed friction back into `docs/industry-coverage-audit.md` / this roadmap
  - **Status (2026-09-03):** actively sourcing a real external candidate — must be a genuine open-source repo (not our own), small enough for a single real PR, with an existing Jest/pytest/Vitest-style test or eval runner for an LLM/chatbot feature, permissive license, and signs of an active maintainer (recent commits/issues). A first web search pass mostly surfaced competing eval platforms (e.g. `getsentry/vitest-evals`, `vercel/next-evals-oss`) rather than adoption targets — those are not candidates. User is reviewing candidates before any repo is touched.
