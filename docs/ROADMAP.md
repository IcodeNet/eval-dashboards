...
- [x] Extend the Promptfoo importer to preserve export-time evaluator context fields that now appear in Promptfoo exports (for example `metadata.sessionId`, grader comments, and latency) instead of dropping them during conversion.
- [x] Map preserved latency to `rows[].durationMs`, keep grader comments in `rows[].reason`, and store source session ids in row metadata for downstream grouping/filtering.
- [x] Add fixture coverage proving these fields survive `eval-dashboards import --from=promptfoo` and pass schema validation + taxonomy lint.
...
- [x] Interop adapter expansion (Ragas, Langfuse, Phoenix, Braintrust, OpenAI eval outputs). Audit 2026-09-15: Ragas, Langfuse, Phoenix, and Braintrust now have real coded adapters, fixtures, and passing tests (`src/cli/import-adapters.ts`, `test/import-adapters-ragas-langfuse.test.ts`, `test/import-adapters-phoenix-braintrust.test.ts`). OpenAI eval outputs now also have a real coded adapter (`--from=openai-evals`, parsing `oaieval` JSONL events logs — `spec`/`match`/`metrics`/`final_report` event types), a fixture, and passing tests (`test/import-adapters-openai-evals.test.ts`). All 5 sources meet the 4B.3 acceptance bar. Done.
