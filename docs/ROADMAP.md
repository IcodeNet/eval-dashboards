...
- [ ] Fix the `RunnerEvalCaseResult` taxonomy-field gap found in the assistant-ui case study — either widen the type to accept `kind`/`agentVersion`/`promptVersion` directly, or document `mapRow` as the expected path up front in `docs/artifact-format.md`
