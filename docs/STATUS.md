...
- Phase 4F (evidence, confidentiality, and org rollout) is in progress. 4F.1 (two-tier artifact split) and 4F.2 (redaction profile and publish preflight) shipped previously. 4F.3 (`eval-check-result/v2` provenance) shipped: `eval-dashboards check --json-v2-out=<path>` writes a v2 payload with resolved gate config, per-suite dataset/rubric versions, sha256 artifact digests, subject commit/release, and CI environment, additive alongside unchanged `eval-check-result/v1` output (`src/cli/index.ts`, `test/cli-check-json-v2-output.test.ts`, `docs/artifact-format.md`). Remaining: 4F.4-4F.11 (digest+signature, waiver register, threshold-change detection, heartbeat verifier, org rollup index, bypass accounting, PR-subset tiering, evidence export bundle). See `docs/ROADMAP.md` Phase 4F. Until 4F.4 ships, do not publish reports produced from production data without a detached signature.
