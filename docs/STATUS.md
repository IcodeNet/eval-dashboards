...
- 4E.3 calibration preflight is complete: when `judge-calibration` is configured, `check` enforces recent calibration evidence for judge-scored suites in blocking mode, warns in report-only mode, excludes current-run self-certification, validates configurable recency (`--calibration-max-age-hours`), and documents explicit override paths (`--allow-stale-calibration`, `--calibration-preflight`, `--no-calibration-preflight`).
- Phase 4F (evidence, confidentiality, and org rollout) is planned but not started. It captures the consensus blockers from the 2026-09-14 four-role review: published reports have no redaction or access control, gate results are self-attested (no resolved config, versions, digests, or signature), and there is no cross-agent rollup, waiver register, or bypass accounting. See `docs/ROADMAP.md` Phase 4F. Until 4F.1-4F.4 ship, do not publish reports produced from production data.
