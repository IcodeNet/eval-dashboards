...
- 4E.3 calibration preflight is complete: when `judge-calibration` is configured, `check` now enforces recent calibration evidence for judge-scored suites in blocking mode, warns in report-only mode, supports configurable recency (`--calibration-max-age-hours`), and documents an explicit escape hatch (`--allow-stale-calibration`).
